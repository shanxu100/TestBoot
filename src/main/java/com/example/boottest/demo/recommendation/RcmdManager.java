...
public class RcmdManager {
    ...
    public void loadDataModel() {
        try {
            ...
            System.out.println("加载DataModel耗时：" + (System.currentTimeMillis() - time1));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public List<RecommendedItem> recommend(long userId, int howMany) throws Exception {
        ...
        //打印推荐的结果
        System.out.println("使用基于用户的协同过滤算法,为用户:" + userId + "推荐 " + howMany + " 个商品。耗时：" + (System.currentTimeMillis() - time1));
